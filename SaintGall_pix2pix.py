...
from SaintGall_data_loader import DataLoader
...
class Pix2Pix():
    def __init__(self):
        ...
        self.dataset_name = 'synthetic_SaintGall'
    ...
    def sample_images(self, epoch):
        ...
        '''
        os.makedirs('pix2pix_SaintGall', exist_ok=True)
        r, c = 3, 3
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig(f"pix2pix_SaintGall/{epoch}.png")
        plt.close()
    ...
    def generate_and_save_test_set(self, output_dir):
        ...
        for idx, (combined_img, _, labels, indices) in enumerate(self.data_loader.load_batch(is_testing=True)):
            ...
            for i in range(len(combined_img)):
                img_name = os.path.join(output_dir, f"{indices[i]}-{labels[i]}.png")
                fake_A_gray = Image.fromarray(fake_A[i]).convert('L')
...
if __name__ == '__main__':
    ...
    print(f"Pix2pix, SaintGall_New, epochs={epochs}, batch_size={batch_size}", flush=True)

    dirpath = '/proj/sciml/users/x_jesst/pix2pix/pix2pix_SaintGall'
