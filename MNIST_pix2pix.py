...
class Pix2Pix():
    def __init__(self):
        ...
        self.dataset_name = 'synthetic_MNIST'
        ...
        valid = self.discriminator([img_A, fake_A])
    ...
    def train(self, epochs, batch_size=128, sample_interval=128):
        ...
        for epoch in range(epochs):
            ...
            for batch_i, (combined_img, undertext_img, _, _) in enumerate(self.data_loader.load_batch()):
                ...
                valid = np.ones((combined_img.shape[0],) + self.disc_patch)  
                fake = np.zeros((combined_img.shape[0],) + self.disc_patch)

                d_loss_real = self.discriminator.train_on_batch([combined_img, undertext_img], valid)
                d_loss_fake = self.discriminator.train_on_batch([combined_img, fake_A], fake)

                d_loss = np.add(d_loss_real, d_loss_fake) / 2

                g_loss = self.combined.train_on_batch([combined_img, undertext_img], [valid, undertext_img])
    ...
    def sample_images(self, epoch):
        ...
        '''
        os.makedirs('pix2pix_MNIST', exist_ok=True)
        r, c = 3, 3
        batch_data = next(self.data_loader.load_batch())

        combined_img, undertext_img, _, _ = batch_data
        ...
        for i in range(r):
            for j in range(c):
                if i == 1:
                    gray_img = Image.fromarray((concat_imgs[cnt] * 255).astype(np.uint8)).convert("L")
                    axs[i,j].imshow(gray_img, cmap="gray")
                else:
                    axs[i,j].imshow(concat_imgs[cnt])
                axs[i,j].set_title(titles[cnt])
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig(f"pix2pix_MNIST/{epoch}.png")
        plt.close()
    ...
    def generate_and_save_test_set(self, output_dir):
        ...
        for idx, (combined_imgs, _, labels, indices) in enumerate(self.data_loader.load_batch(is_testing=True)):

            fake_combined = self.generator.predict(combined_imgs)
            fake_combined = 0.5 * fake_combined + 0.5 
            fake_combined = (fake_combined * 255).astype(np.uint8)

            for i in range(len(combined_imgs)):
                img_name = os.path.join(output_dir, f"{indices[i]}_label{labels[i]}.png")
                fake_combined_gray = Image.fromarray(fake_combined[i]).convert('L')
                fake_combined_gray.save(img_name)
            print(f"Processed batch {idx + 1}")


def create_gt_file(output_dir, gt_filename):
    '''
    Greate .gt file with labels for generated images (to be used by AttentionHTR)
    '''
...
if __name__ == '__main__':
    ...
    gan.train(epochs, batch_size, sample_interval=batch_size)
    print(f"Pix2pix, MNIST, epochs={epochs}, batch_size={batch_size}", flush=True)

    dirpath = '/proj/sciml/users/x_jesst/pix2pix/pix2pix_MNIST'
    gan.generate_and_save_test_set(dirpath)
    create_gt_file(dirpath, "gt.txt")
