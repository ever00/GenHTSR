...
class DataLoader:
    ...
    def load_data(self, is_testing):
        ...
        for idx, filename in enumerate(os.listdir(combined_path)):
            ...
            combined_img = np.array((combined_img).resize(self.img_res))
            undertext_img = np.array((undertext_img).resize(self.img_res))

            combined_img = self.normalize_image(combined_img)
            undertext_img = self.normalize_image(undertext_img)
    ...
    def load_batch(self, is_testing=False):
        ...
        for i in range(0, len(data), self.batch_size):
            ...
            imgs_combined, imgs_undertext, labels, indices = zip(*batch_data)
            imgs_combined, imgs_undertext = np.array(imgs_combined), np.array(imgs_undertext)
            yield imgs_combined, imgs_undertext, labels, indices