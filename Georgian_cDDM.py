...
def plot_progress(combined_img, undertext_img, output_dir, epoch):
    '''
    Create and save plots from the models current performance
    '''
    model.eval() 
    ...
    with torch.no_grad(): 
        ...
        axes[1].set_title("GT Undertext")

        axes[2].imshow(np.array(Image.fromarray((np.transpose(output_img.squeeze(), (1, 2, 0)) * 255).astype(np.uint8))))
        axes[2].set_title("Generated Undertext")
...
def generate_and_save_test_set(output_dir):
    '''
    Generate clean under-texts from the combined test set
    '''
    model.eval()
    os.makedirs(output_dir, exist_ok=True)

    for idx, (combined_img, _, indices) in enumerate(data_loader.load_batch(is_testing=True)):
...
T, num_epochs, lr = 30, 5, 0.0001
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
output_dir = 'Georgian_cDDM'

model = ConditionalDiffusionUNet().to(device)
optimizer = torch.optim.AdamW(model.parameters(), lr=lr)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=5)
data_loader = DataLoader(dataset_name='synthetic_Georgian', img_res=(224, 224), batch_size=64)
print(f"cDDM, Georgian, epochs={num_epochs}, T={T}", flush=True)

for epoch in range(num_epochs):
    for idx, (combined_img, undertext_img, original_idx) in enumerate(data_loader.load_batch(is_testing=False)):
        combined_img = torch.tensor(combined_img, dtype=torch.float32).permute(0, 3, 1, 2).to(device)
