...
def denoise_image2(model, combined_img, T, device):
    ...
    threshold = 0.8  # Set threshold for heuristic denoising sensitivity
...
def plot_progress(combined_img, undertext_img, output_dir, epoch):
    '''
    Create and save plots from the models current performance (during training)
    '''
    model.eval() 
    ...
    with torch.no_grad(): 
        ...
        axes[0].set_title("Generated Undertext")
...
# Load data
data_loader = DataLoader(dataset_name='synthetic_SaintGall', img_res=(224, 224), batch_size=64)

# Train model
for epoch in range(num_epochs):
     
    # Load batch
    for idx, (combined_img, undertext_img, labels, original_idx) in enumerate(data_loader.load_batch(is_testing=False)):
        combined_img = torch.tensor(combined_img, dtype=torch.float32).permute(0, 3, 1, 2).to(device)
    ...
    # Learnig rate scheduler takes a step
    scheduler.step()
